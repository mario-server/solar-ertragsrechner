...
function DayPage({ settings, date, setDate, day, weatherSource }: { settings: Settings; date: string; setDate: (d: string) => void; day: DayResult; weatherSource: WeatherData['source'] }) {
  ...
  const series = sides.flatMap((side, index) => [{ key: `surface${index}Ideal`, label: `${side.name || `Fläche ${index + 1}`} ideal`, color: surfaceColor(index), real: false }, { key: `surface${index}Real`, label: `${side.name || `Fläche ${index + 1}`} realistisch`, color: surfaceColor(index), real: true }])
  const chartLabels: Record<string, string> = { totalIdeal: 'Gesamt ideal', totalReal: 'Gesamt realistisch' }
  const chartColors: Record<string, string> = { totalIdeal: COLORS.total, totalReal: COLORS.real }
  ...
  const defaultVisible = [...series.filter((item) => !item.real && sides[Number(item.key.match(/\d+/)?.[0] ?? 0)].active).map((item) => item.key), 'totalIdeal', 'totalReal']
  ...
  const chartPoints = day.points.filter((_, i) => i % 3 === 0).map((p) => {
    const point: Record<string, number | string> = { time: p.timeLabel, totalIdeal: p.totalIdeal, totalReal: p.totalReal }
    sides.forEach((_, index) => { point[`surface${index}Ideal`] = p.sideIdeal[index] ?? 0; point[`surface${index}Real`] = p.sideReal[index] ?? 0 })
    return point
  })
  const chartKeys = [...series.filter((item) => sides[Number(item.key.match(/\d+/)?.[0] ?? 0)].active).map((item) => item.key), 'totalIdeal', 'totalReal']
  ...
  const rows = day.points.map((p, index) => <tr key={p.timeLabel}><td>{p.timeLabel}</td><td>{n(p.elevation, 1)}°</td><td>{n(p.azimuth, 1)}°</td>{sides.map((side, sideIndex) => <Fragment key={side.id || sideIndex}><td>{n((p.sideIdeal[sideIndex] ?? 0) * 1000, 0)} W</td><td>{n((p.sideReal[sideIndex] ?? 0) * 1000, 0)} W</td></Fragment>)}<td className="strong-cell">{n(p.totalIdeal * 1000, 0)} W</td><td className="strong-cell">{n(p.totalReal * 1000, 0)} W</td><td className="accumulated-cell">{n(accumulated[index], 3)} kWh</td>{showTechnical && <><td>{n(p.incidence, 1)}°</td><td>{n(p.irradiance.poa, 0)} W/m²</td></>}</tr>)
  const csvHeader = ['Uhrzeit', 'Sonnenhöhe', 'Sonnenazimut', ...sides.flatMap((side, index) => [`${side.name || `Fläche ${index + 1}`} ideal W`, `${side.name || `Fläche ${index + 1}`} realistisch W`]), 'Gesamt ideal W', 'Gesamt realistisch W', 'Gesamt realistisch aufgelaufen kWh']
  const download = () => downloadCsv(`solar-tag-${date}.csv`, [csvHeader, ...day.points.map((p, index) => [p.timeLabel, p.elevation, p.azimuth, ...sides.flatMap((_, sideIndex) => [(p.sideIdeal[sideIndex] ?? 0) * 1000, (p.sideReal[sideIndex] ?? 0) * 1000]), p.totalIdeal * 1000, p.totalReal * 1000, accumulated[index]])])
  return <main><div className="page-heading compact"><div><span className="eyebrow">Tagesanalyse</span><h1>Leistung im Tagesverlauf</h1><p>5-Minuten-Modell mit getrennten idealen und klimatisch realistischen Werten für alle konfigurierten Flächen.</p></div><div className="date-control"><IconButton title="Vorheriger Tag" onClick={() => move(-1)}>←</IconButton><input type="date" value={date} onChange={(e) => setDate(e.target.value)} /><IconButton title="Nächster Tag" onClick={() => move(1)}>→</IconButton><button className="secondary" onClick={() => setDate(today())}>Heute</button></div></div><div className="metric-grid day-metrics"><Metric label="Gesamt ideal" value={kwh(day.energy.totalIdeal)} detail={`Peak ${kw(day.peak.ideal)} · ${day.peak.idealTime}`} accent="orange" /><Metric label="Gesamt realistisch" value={kwh(day.energy.totalReal)} detail={`Peak ${kw(day.peak.real)} · ${day.peak.realTime}`} accent="teal" /><Metric label="Aufgelaufen realistisch" value={kwhPrecise(day.energy.totalReal)} detail="über den gesamten Tag integriert" accent="teal" />{sides.slice(0, 2).map((side, index) => <Metric key={side.id || index} label={side.name || `Dachfläche ${index + 1}`} value={kwh(day.sideEnergy[index]?.real ?? 0)} detail={`Ideal ${kwh(day.sideEnergy[index]?.ideal ?? 0)}`} accent={index === 0 ? 'orange' : 'teal'} />)}</div><section className="panel large-chart"><div className="section-title"><div><span>Leistungsprofil · kW</span><h2>Alle Flächen im direkten Vergleich</h2></div><div className="chart-toggles">{chartKeys.map((key) => <button key={key} className={visible.includes(key) ? 'selected' : ''} onClick={() => toggle(key)}><i style={{ background: chartColors[key] }} />{chartLabels[key]}</button>)}</div></div><LineChart points={chartPoints} keys={visible} labels={chartLabels} colors={chartColors} height={330} /></section><section className="panel"><div className="table-head"><div><span className="eyebrow">Zeitreihe</span><h2>Alle Berechnungspunkte</h2></div><div className="table-actions"><label><input type="checkbox" checked={showTechnical} onChange={(e) => setShowTechnical(e.target.checked)} /> Technische Spalten</label><button className="secondary" onClick={download}>CSV exportieren</button></div></div><div className="scroll-table"><table><thead><tr><th>Uhrzeit</th><th>Sonnenhöhe</th><th>Sonnenazimut</th>{sides.map((side, index) => <Fragment key={side.id || index}><th>{side.name || `Fläche ${index + 1}`} ideal W</th><th>{side.name || `Fläche ${index + 1}`} real W</th></Fragment>)}<th>Gesamt ideal W</th><th>Gesamt real W</th><th>Aufgelaufen real kWh</th>{showTechnical && <><th>Einfallswinkel</th><th>POA</th></>}</tr></thead><tbody>{rows}</tbody></table></div><small className="table-note">Datenquelle realistisch: {weatherSource === 'PVGIS' ? 'PVGIS / EU Joint Research Centre' : 'monatliche Klimaschätzung als Fallback'}. Intern werden Leistungen in kW gerechnet, hier für die Zeitreihe in W angezeigt. Die kumulierte Energie wird aus der realistischen Gesamtleistung integriert. Diagramm, Tabelle und Summen berücksichtigen alle aktiven Flächen.</small></section></main>
}
...
function YearPage({ settings, rows, year, setYear, progress, clippingKwh, calculating }: { settings: Settings; rows: YearRow[]; year: number; setYear: (y: number) => void; progress: number; clippingKwh: number; calculating: boolean }) {
  const total = rows.reduce((s, r) => s + r.totalReal, 0); const ideal = rows.reduce((s, r) => s + r.totalIdeal, 0); const totalKwp = configuredRoofSides(settings).reduce((sum, side) => sum + (side.active ? side.powerKwp : 0), 0); const monthly = Array.from({ length: 12 }, (_, month) => rows.filter((r) => Number(r.date.slice(5, 7)) - 1 === month).reduce((s, r) => s + r.totalReal, 0)); const monthRows = monthly.map((value, i) => ({ month: ['Jan', 'Feb', 'Mär', 'Apr', 'Mai', 'Jun', 'Jul', 'Aug', 'Sep', 'Okt', 'Nov', 'Dez'][i], value, ideal: rows.filter((r) => Number(r.date.slice(5, 7)) - 1 === i).reduce((s, r) => s + r.totalIdeal, 0) }))
